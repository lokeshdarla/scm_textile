...
contract SupplyChainPhase1  {
  ...
  mapping(uint256 => RawMaterial) private rawMaterialOfFarmer;
  mapping(uint256 => RawMaterial[]) private farmerRawMaterials;
  mapping(uint256 => Fabric) private fabricOfMill;
  mapping(uint256 => Fabric[]) private millFabrics;
  mapping(address => uint256) private userAddressToFarmerId;
  mapping(address => uint256) private userAddressToMillId;
  mapping(address => User) private userDetails;
  ...
  function addRawMaterial(
    ...
    _rawMaterialIdCounter++;
    rawMaterialOfFarmer[_rawMaterialIdCounter] = RawMaterial({
      id: _rawMaterialIdCounter,
      farmerId: _farmerId,
      materialType: _materialType,
      quantity: _quantity,
    ...
    farmerRawMaterials[_farmerId].push(rawMaterialOfFarmer[_rawMaterialIdCounter]);
  ...
  function getRawMaterial(uint256 _rawMaterialId) external view onlyFarmer returns (RawMaterial memory) {
    return rawMaterialOfFarmer[_rawMaterialId];
  }

  function getAllRawMaterials() external view onlyFarmer returns (RawMaterial[] memory rawMaterials) {
    uint256 _farmerId = userAddressToFarmerId[msg.sender];
    rawMaterials = farmerRawMaterials[_farmerId];
    require(rawMaterials.length > 0, "No raw materials found");
    return rawMaterials;
  }
  ...
  function addFabric(
    ...
    rawMaterialOfFarmer[_rawMaterialId].isAvailable = false;
    _fabricIdCounter++;

    fabricOfMill[_fabricIdCounter] = Fabric({
      id: _fabricIdCounter,
      millId: _millId,
      rawMaterialId: _rawMaterialId,
      quantity: _quantity,
    ...
    millFabrics[_millId].push(fabricOfMill[_fabricIdCounter]);
  ...
  function getFabric(uint256 _fabricId) external view onlyMill returns (Fabric memory fabric) {
    fabric = fabricOfMill[_fabricId];
    require(fabric.isAvailable, "Fabric not available");
    return fabric;
  }

  function getAllFabrics() external view onlyMill returns (Fabric[] memory fabrics) {
    uint256 _millId = userAddressToMillId[msg.sender];
    fabrics = millFabrics[_millId];
    require(fabrics.length > 0, "No fabrics found");
    return fabrics;
  }
}
